 ...
 *
 */
const getPowerFunction = (exponent) => {
  const exp = exponent;
  return (num) => num ** exp;
};
...
const logger = (func, logFunc) => (...args) => {
  ...
  const loggingString = `${func.name}(${argsString.substr(1, argsString.length - 2)})`;

  logFunc(`${loggingString} starts`);

  const res = func(...args);

  logFunc(`${loggingString} ends`);
